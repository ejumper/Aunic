...
// padTo pads or truncates s to exactly width visual cells. Accounts for ANSI
// escape sequences by using ansiVisualWidth.
func padTo(s string, width int) string {
	...
	if w > width {
		// Truncate — walk runes counting visual width.
		var b strings.Builder
		vis := 0
		inEsc := false
		for _, r := range s {
...
// ansiVisualWidth returns the visual cell width of s, skipping ANSI escapes.
// Duplicated here so findbar.go has no import cycle with render.go.
